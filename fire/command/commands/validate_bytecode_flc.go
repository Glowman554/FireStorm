...
type ValidateBytecodeFlc struct{}

func (ValidateBytecodeFlc) PopulateParser(parser *arguments.Parser) {
}

func (ValidateBytecodeFlc) Execute(parser *arguments.Parser) error {
	passed := 0
	notPassed := 0
	extension := "flbb"
	...
	err := filepath.Walk(".", func(path string, info os.FileInfo, err error) error {
		if err == nil {
            ...
            run("flc", []string{path, "--output="+path + "." + extension, "--input="+path, "--include=../libraries/stdlib/"})
...
func (ValidateBytecodeFlc) Description() string {
