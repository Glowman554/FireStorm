...
void reverse(Parser* p) {
    if (p->pos > 0) {
        p->pos--;
    }
}

void parser_error(Parser* p, const char* message, int pos) {
...
Node* node_new(NodeType type, Node* a, Node* b, void* value, int pos) {
...
int is_datatype_string(const char* str) {
...
Datatype string_to_datatype(const char* str) {
    ...
    if (strcmp(str, "int32") == 0 || strcmp(str, "i32") == 0) {
        return DATATYPE_INT_32;
    }
    if (strcmp(str, "int16") == 0 || strcmp(str, "i16") == 0) {
        return DATATYPE_INT_16;
    }
    return DATATYPE_INT;
}

Variable parse_datatype_named(Parser* p) {
    Variable var;
    Token* tok = current_token(p);
...
void parse_datatype_unnamed(Parser* p, Datatype* dt, int* is_array) {
    Token* tok = current_token(p);
            ...
            if (tok->type != TOKEN_RPAREN) {
                while (1) {
                    ...
                    if (arg_count >= arg_capacity) {
                        ...
                        args = realloc(args, arg_capacity * sizeof(Node*));
                    }
                    args[arg_count++] = expr;
...
// Parse a code block
Node** parse_code_block(Parser* p, int* body_count) {
    Node** body = NULL;
    int count = 0;
    int capacity = 0;
    ...
    while (1) {
        ...
        if (stmt) {
            if (count >= capacity) {
                ...
                body = realloc(body, capacity * sizeof(Node*));
            }
            body[count++] = stmt;
        }
    ...
    if (tok->type == TOKEN_ID) {
        ...
        if (strcmp(tok->value, "range") == 0) {
            ...
            *one_val = 1;
            ...
            Node* update_expr = is_up ? node_new(NODE_ADD, var_lookup2, one, NULL, pos) : node_new(NODE_SUBTRACT, var_lookup2, one, NULL, pos);
...
void parse_function_params(Parser* p, Variable** params, int* param_count) {
    *params = NULL;
    *param_count = 0;
    int capacity = 0;
    ...
    while (1) {
        ...
        if (*param_count >= capacity) {
            ...
            *params = realloc(*params, capacity * sizeof(Variable));
        }
        ...
        if (tok->type == TOKEN_ID) {
            ...
            if (is_datatype_string(tok->value)) {
                ...
                *var_ptr = var;
                ...
                if (count >= capacity) {
                    capacity = capacity == 0 ? 8 : capacity * 2;
                    global = realloc(global, capacity * sizeof(Node*));
                }
                global[count++] = decl;
            }
