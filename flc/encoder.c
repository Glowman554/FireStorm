...
// Datatypes
const char* datatypes[] = {"int", "chr", "str", "ptr", "i16", "i32"};
const int num_datatypes = sizeof(datatypes) / sizeof(datatypes[0]);

// Native functions
const char* native_functions[] = {
    "exit",
    "putchar", "puts",
    "malloc", "free",
    "fopen", "fclose", "fseek", "fread", "fwrite", "ftell"};
const int num_native_functions = sizeof(native_functions) / sizeof(native_functions[0]);

// Helper function to find index
int find_index(const char** array, int size, const char* item) {
...
void add_global(SymbolTable* st, const char* name) {
...
void add_function(SymbolTable* st, const char* name) {
...
int find_global(SymbolTable* st, const char* name) {
    return find_index((const char**)st->globals, st->global_count, name);
}

int find_function(SymbolTable* st, const char* name) {
    return find_index((const char**)st->functions, st->function_count, name);
}

// Section info
typedef enum { SECTION_FUNCTION,
               SECTION_GLOBAL } SectionType;
...
Section** parse_sections(const char* code, int* section_count) {
...
// Escape character
unsigned char escape_char(char c) {
    switch (c) {
    case '"':
        return '"';
    case 'n':
...
// Translate function
char* translate_function(Section* section, SymbolTable* st) {
    ...
    for (int i = 0; i < section->body_count; i++) {
        ...
        if (strcmp(inst, "variable") == 0 || strcmp(inst, "global_reserve") == 0) {
            ...
            int native_id = find_index(native_functions, num_native_functions, arg1);
            snprintf(buffer, sizeof(buffer), "\t\tdq %d\n", native_id);
            sb_append(sb, buffer);
...
char* translate_global(Section* section, SymbolTable* st) {
