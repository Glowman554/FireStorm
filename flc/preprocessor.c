...
int is_file_included(Preprocessor* preprocessor, const char* filename) {
...
void add_included_file(Preprocessor* preprocessor, const char* filename) {
    preprocessor->included_files = realloc(preprocessor->included_files, (preprocessor->included_file_count + 1) * sizeof(char*));
        preprocessor->included_files[preprocessor->included_file_count] = string_duplicate(filename);
    preprocessor->included_file_count++;
}

char* try_read(Preprocessor* preprocessor, const char* filename) {
...
char* find_pattern(const char* str, const char* pattern_start, const char* pattern_end, char** content, int* start_pos, int* end_pos) {
...
char* string_replace_first(const char* str, int start, int end, const char* replacement) {
...
char* string_replace_all(const char* str, const char* find, const char* replace) {
    if (!str || !find) {
...
void free_define_list(DefineList* list) {
...
char* process_includes(Preprocessor* preprocessor, char* code);

char* process_includes_impl(Preprocessor* preprocessor, char* code) {
...
char* process_natives(char* code) {
    char* result = code;
    char* content;
    int start_pos, end_pos;
...
char* process_defines(char* code) {
...
char* preprocessor_process(Preprocessor* preprocessor, char* code) {
    ...
    if (after_native != after_include) {
        free(after_native);
    }
    if (after_include != after_use) {
        free(after_include);
    }
    if (after_use != code) {
        free(after_use);
